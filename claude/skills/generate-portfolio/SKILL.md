...
5. **Check for Demo Maker output:** If `.demo-maker/` exists, find the latest demo run folder in `OUTPUT/` (pattern: `OUTPUT/demo-YYYYMMDD-HHMMSS/`). If `demo-full.mp4` exists in that folder, it will be copied into the output directory and embedded with a relative path in Step 5b.
...
If Demo Maker `demo-full.mp4` was found in Step 1.5:
